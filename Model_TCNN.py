...
def Model_TCNN(train_data, train_target, test_data, test_target, sol=None):
    if sol is None:
        sol = 50
    IMG_SIZE = 20
    ...
    pred_val_y, pred_test_y, best_score = train_pred1(Train_X, train_target, Test_X, test_target, sol)

    Eval = evaluation(pred_val_y, test_target)
    return Eval, best_score


def Model_DTCNN_feat(data, tar, sol='None'):
    if sol == 'None':
        sol = [1, 2]
    IMG_SIZE = 20
    ...
    for i in range(data.shape[0]):
        ...
        Train_X[i] = np.reshape(int(temp), (IMG_SIZE, IMG_SIZE, 1))

    Test_X = np.zeros((data.shape[0], IMG_SIZE, IMG_SIZE, 1))
    for i in range(data.shape[0]):
        temp = np.resize(data[i], (IMG_SIZE * IMG_SIZE, 1))
        Test_X[i] = np.reshape(temp, (IMG_SIZE, IMG_SIZE, 1))
    pred_val_y, pred_test_y, best_score = train_pred1(Train_X, tar, Test_X, tar, sol)

    return best_score



def model_tcn1(embedding_matrix):
    inp = Input(shape=(maxlen,))
    x = Embedding(max_features, embed_size, weights=[embedding_matrix], trainable=False)(inp)
    x = SpatialDropout1D(0.1)(x)
    x = TCN(128, dilations=[1, 2, 4], return_sequences=True, activation='wavenet', name='tnc1')(x)
    ...
    conc = concatenate([avg_pool, max_pool])
    conc = Dense(16, activation="relu")(conv)
    conc = Dropout(0.1)(conc)
    outp = Dense(1, activation="sigmoid")(conc)
...
def train_pred(train_X, train_y, val_X, val_y, sol, callback=None):
    for e in range(epochs=2):
        model_tcn.fit(train_X, train_y, batch_size=4, epochs=int(sol[0]), validation_data=(val_X, val_y),
                      callbacks=callback, verbose=True)
        pred_val_y = model_tcn.predict([val_X], batch_size=4, verbose=0)
    ...
    pred_test_y = model_tcn1.predict([train_X], batch_size=4, verbose=0)
    print('=' * 60)
    return pred_val_y, pred_test_y, best_score


def Model_MTCNN_AM(train_data, train_target, test_data, test_target, sol=None):
    if sol is None:
        sol = 50
    IMG_SIZE = 20
    Train_X = np.zeros((train_data.shape[0], IMG_SIZE, IMG_SIZE, 1))
    ...
    for i in range(test_data.shape[0]):
        temp = np.resize(test_data[i], (IMG_SIZE * IMG_SIZE, 1))
        Test_X[i] = np.reshape(temp, (IMG_SIZE, IMG_SIZE, 1))
    pred_val_y, pred_test_y, best_score = train_pred(Train_X, train_target, Test_X, test_target, sol)
